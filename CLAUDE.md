...
- **宿主工具白名单与 role-guard 串联分工**（CR-020 R2）：工具能否被使用由宿主 `agents/*.md` 的 `tools:` 裁决（未声明即不可用），写入落到哪个路径由 role-guard 裁决，同一约束不在两处独立声明；本节不复述细则，**单一权威记录在 `docs/kb/domains/guards.md`「宿主原生能力与 role-guard 的分工」**。Thinker 未被授予 `Bash`（工具不存在，通道彻底关闭）；**Worker/Verifier 持裸 `Bash`，其命令形态不受任何强制点约束——该缺口未关闭，宿主的 `permissions` 与 hook matcher 两条路线已评估未采用（理由见 guards.md），已如实登记**（实测：`agents/*.md` 的 `tools:` 丢弃 `Bash(pattern)` 的参数部分，写参数模式等于授予完整 Bash）
- **`tools:` 的强制力需三段链路齐备**（CR-020 R1 + repair 1）：声明（`agents/*.md` frontmatter）+ 发现面（`.claude/agents/{thinker,worker,verifier}.md` **symlink 指向 `agents/` 同名文件**，宿主项目级发现路径）+ 派发链（四个 `workflows/*.js` 的 `agent()` 传 `agentType`，自用取裸名）。缺任一条宿主即套用内置 `workflow-subagent`（`tools: ["*"]`），声明成**死声明**——round 0 曾只写声明就在文档记为「已归位」，这与本 CR 要消除的「软性替代冒充强制点」同形态。⛔ `.claude/agents/` 不得改成手工副本（两份定义会漂移，而实际强制点取 `.claude/agents/` 那一份）；`scripts/check-harness.sh` 机械校验 symlink 同源与 `agentType` 齐备
- `.claude/settings.json` 注册两个 hook 事件：`PreToolUse`（`Write|Edit|NotebookEdit` → `role-guard.sh`，判权，可 `exit 2`）与 `SessionStart`（→ `scripts/session-context.sh`，**sensor**：只读状态、只打印、恒 `exit 0`，不判权不阻断，故默认会话不受影响）
- 仓库根即插件根：`.claude-plugin/plugin.json` + 已在根的 `agents/`、`skills/`、`.claude/commands/` 构成可分发插件形态，**未迁移任何目录**，`skills/mh-*` 路径引用一律不变；`tools/mh-dev/` 不属插件组件目录，自开发流程不进入分发面
