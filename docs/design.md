...
> **核心原则：脚本硬约束优先于自然语言软约束。** 凡是可程序化验证的规则，必须用脚本实现，不依赖 Agent 自述。
...
| verify-ppt.sh | PPT 专项校验 | viewport、.slide容器、CSS引用、页数一致性、占位符残留 |

**约束升级路径：** 自然语言约束 → 发现频繁违反 → 脚本化 → 硬性拦截

详见：scripts/verify.sh, scripts/verify-qa.sh, scripts/verify-ppt.sh
