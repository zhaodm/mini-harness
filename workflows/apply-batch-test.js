/**
 * apply-batch-test.js — Apply 阶段 Batch 内 Verifier 并行审计
 *
 * 触发时机: Batch 内所有 Worker Task 通过质量门禁后，并行派发 Verifier 审计。
 ...
 *     prompt  — Verifier SubAgent 的完整 prompt（角色契约 + handoff 内容）
 *
 * 返回:
 *   { batchId, tasks: [{ taskId, result, passed }] }
 *   Orchestrator 根据 passed 字段判断是否进入修复循环。
 */
export const meta = {
  name: "apply-batch-test",
  description: "Apply 阶段 Batch 内 Verifier 并行审计",
  phases: ["batch-test"]
};
