/**
 * apply-final-audit.js — Apply 阶段 Verifier 最终全量审计
 *
 * 触发时机: SR2 功能评审通过后，派发 Verifier 做最终全量审计（E2E + 回归 + 工程验证）。
 * 调用方: Orchestrator 主会话通过 Workflow 工具调用。
 *
 * args:
 *   reqId  — 需求编号 (e.g., "REQ003")
 *   prompt — Verifier SubAgent 的完整 prompt（角色契约 + 最终审计 handoff）
 *
 * 返回:
 *   { result, passed }
 *   Orchestrator 根据 passed 判断进入 SR3 或修复循环。
 */
export const meta = {
  name: "apply-final-audit",
  description: "Apply 阶段 Verifier 最终全量审计",
  phases: ["final-audit"]
};
