 ...
 * @property {string} [activeReqId] - RESUME 场景下的活跃 REQ-ID
 */
...
export function detectScenario(input) {
  const { globalStateExists, reqStatePhase, outputSpecFiles, activeReqId } = input;
  ...
  if (reqStatePhase && reqStatePhase !== 'done') {
    return {
      ...
      activeReqId: activeReqId || undefined
    };
  }
