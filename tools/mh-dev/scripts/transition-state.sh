#!/bin/bash
set -euo pipefail
ROOT_DIR="$(cd "$(dirname "$0")/../../.." && pwd)"; RUNTIME="${MH_DEV_RUNTIME:-$ROOT_DIR/tools/mh-dev/.mh-dev}"; STATE="$RUNTIME/state.json"; NEXT="${1:-}"; shift || true
...
if next_=='repair':
 r=s.setdefault('repair',{}); r['round']=r.get('round',0)+1;r['status']='active';r['reason']=reason or 'verification_failure';r['source_verdict']='evidence/test-verdict.json'
